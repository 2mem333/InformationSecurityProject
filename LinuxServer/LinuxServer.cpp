...
{
    // Hızlı mapping: find ile de olur ama bu daha net
...
{
    // Eğer "data:image/png;base64,...." gibi geldiyse header kırp
...
bool checkPassword(std::string name, std::string password)
{
    ...
    while (file >> fileName >> filePassword)
    {
        ...
        {
            file.close();
            return true; // doğru kullanıcı
        }
    }

    file.close();
    return false; // bulunamadı veya yanlış
}
...
std::string listUsers()
{
    std::ifstream file("databases/users.txt");
    if (!file.is_open())
        return ""; // dosya açılamadıysa boş string
...
std::string getMails(const std::string& name)
{
    ...
    while (std::getline(file, line))   // satır satır oku
    {
        if (line.empty())
            continue;
...
std::string DesKeyForUser(const std::string& name)
{
    ...
    // 2) Sezar kaydırma (printable ASCII 32..126 içinde kalsın)
    // shift değerini pwd uzunluğundan türetiyoruz (deterministik)
...
{
    // Mesaj receiver'ın anahtarı ile şifrelenir
    std::string RecKey = DesKeyForUser(receiver); // receiver anahtarı
    std::string encMsg = simple_des::DES_EncryptBase64(msg, RecKey);
    std::cout << "encrypted msg (with key of rec): " << encMsg << "\n";

    // sender:mesaj formatı
    ...
    if (inFile.is_open())
    {
        ...
        // Dosyada boş satır yoksa mesajı sona ekle
        if (!written)
        {
            lines.push_back(finalMsg);
        }
    ...
    {
        // Dosya yoksa oluştur ve ilk satıra yaz
        lines.push_back(finalMsg);
    }
    ...
    while (true) {
        ...
        if (json_get_value(req, "type", type)) {
            ...
            else if(type == "LOGIN")
            {
                ...
                //burada veritabanında kontrol yapcak sonra cccevap göndericek..
